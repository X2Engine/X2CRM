<?php return array (
  'No relationships found.' => 'Никаких отношений не найдено.',
  'Such a relationship already exists.' => 'Такая связь уже существует.',
  'Successfully saved a relationship.' => 'Успешно сохранены отношения.',
  'Failed to save relationship record for unknown reason.' => 'Не удалось сохранить запись связи по неизвестной причине.',
  '{n} relationships deleted.' => '{n} отношения удалены.',
  'One or more relationships could not be deleted.' => 'Один или больше отношений не может быть удален.',
  'Request type not supported for this action.' => 'Тип запроса не поддерживается для этого действия.',
  'Tags not added.' => 'Тэги не добавляют.',
  'Error' => 'Ошибка',
  'Cannot delete; no parameters specified for finding a relationship record to delete.' => 'Не удается удалить; никакие параметры не указаны для нахождения запись связи удалить.',
  'No relationships deleted; none were found matching specfied parameters.' => 'Не отношения исключить; не было найдено ни specfied соответствующие параметры.',
  'Couldn\'t perform {a} on model {m}' => 'Не мог \\ &#39;т выполнить {a} от модели {m}',
);
