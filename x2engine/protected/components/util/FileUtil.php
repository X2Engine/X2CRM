...
class FileUtil {
     ...
     * @return boolean
     */
    public static function ccopy($source, $target, $relTarget = false, $ds=DIRECTORY_SEPARATOR){
        $target = self::rpath($target);
        if($relTarget)
        ...
        if(preg_match('%^https?://%', $source)){
            ...
            if(is_dir($source)){
                ...
                foreach($files as $file){
                    if($file != '.' && $file != '..'){
                        $return = $return && FileUtil::ccopy($source.$ds.$file, $target.$ds.$file,$ds);
                    }
                }
                return $return;
            }else{
