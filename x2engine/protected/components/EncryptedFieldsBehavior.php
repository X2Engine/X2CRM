...
class EncryptedFieldsBehavior extends TransformedFieldStorageBehavior {
	...
	/**
	 * Constructor; checks for whether the encryption utility is available.
	 * 
	 * @throws Exception 
	 */
	...
	public function packAttribute($name){
		return self::$encryption->encrypt($this->getOwner()->$name);
	}
	...
	public function unpackAttribute($name){
		if($this->getOwner()->$name)
			return self::$encryption->decrypt($this->getOwner()->$name);
		else
			return null;
	}
}
