...
class X2List extends X2Model {
    ...
    public function behaviors(){
        return array(
            ...
            'LinkableBehavior' => array(
                ...
                'baseRoute' => '/accounts/accounts/list',
                'autoCompleteSource' => '/accounts/accounts/getLists',
            ),
    ...
    public function attributeLabels(){
        return array(
            'id' => 'ID',
            'assignedTo' => Yii::t('accounts', 'Owner'),
            'name' => Yii::t('accounts', 'Name'),
            'description' => Yii::t('accounts', 'Description'),
            'type' => Yii::t('accounts', 'Type'),
            'logicType' => Yii::t('accounts', 'Logic Type'),
            'modelName' => Yii::t('accounts', 'Record Type'),
            'visibility' => Yii::t('accounts', 'Visibility'),
            'count' => Yii::t('accounts', 'Members'),
    ...
    public static function getComparisonList(){
        return array(
            '=' => Yii::t('accounts', 'equals'),
            '>' => Yii::t('accounts', 'greater than'),
            '<' => Yii::t('accounts', 'less than'),
            '<>' => Yii::t('accounts', 'not equal to'),
            'contains' => Yii::t('accounts', 'contains'),
            'noContains' => Yii::t('accounts', 'does not contain'),
            'empty' => Yii::t('accounts', 'empty'),
            'notEmpty' => Yii::t('accounts', 'not empty'),
            'list' => Yii::t('accounts', 'in list'),
            'notList' => Yii::t('accounts', 'not in list'),
        );
    }
    ...
    public function getDisplayName ($plural=true, $ofModule=true) {
        return Yii::t('accounts', '{contact} Lists|{contact} List', array(
            (int) $plural,
            '{contact}' => Modules::displayName(false, 'Accounts'),
        ));
    }
    ...
    public function calculateCount () {
        ...
        return Accounts::model ()->count ($criteria);
    }
    ...
    public function queryCriteria($useAccessRules = true){
        ...
        if($useAccessRules){
            $accessCriteria = X2Model::model('Accounts')->getAccessCriteria(); // record-level access control for Accounts
...
//        );
     ...
     * @param bool $allowNewsletter whether to allow accounts to be added to a newsletter
     */
    ...
    public static function getRoute($id){
        if($id == 'all')
            return array('/accounts/accounts/index');
        else if($id == 'new')
            return array('/accounts/accounts/newAccounts');
        else if(empty($id) || $id == 'my')
            return array('/accounts/accounts/myAccounts');
        else
            return array('/accounts/accounts/list', 'id' => $id);
    }
    ...
    public function setAttributes($values, $safeOnly = true){
        if($this->type == 'dynamic'){
            ...
            if(count($this->criteriaInput['attribute']) !== count($this->criteriaInput['value'])){
                $this->addError('', Yii::t('accounts','Invalid list criteria.'));
            }else{
                ...
                if (array_key_exists ('tags', $criteria) && empty($criteria['tags'])) {
                    $this->addError ('tags', Yii::t ('accounts', 'Tag list must be non-empty'));
                }
            }
        }
