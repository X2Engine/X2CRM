...
class X2List extends X2Model {
    ...
    public function behaviors(){
        return array(
            ...
            'LinkableBehavior' => array(
                ...
                'baseRoute' => '/opportunity/opportunity/list',
                'autoCompleteSource' => '/opportunity/opportunity/getLists',
            ),
    ...
    public function attributeLabels(){
        return array(
            'id' => 'ID',
            'assignedTo' => Yii::t('opportunity', 'Owner'),
            'name' => Yii::t('opportunity', 'Name'),
            'description' => Yii::t('opportunity', 'Description'),
            'type' => Yii::t('opportunity', 'Type'),
            'logicType' => Yii::t('opportunity', 'Logic Type'),
            'modelName' => Yii::t('opportunity', 'Record Type'),
            'visibility' => Yii::t('opportunity', 'Visibility'),
            'count' => Yii::t('opportunity', 'Members'),
            'createDate' => Yii::t('opportunity', 'Create Date'),
            'lastUpdated' => Yii::t('opportunity', 'Last Updated'),
        );
    }
    ...
    public static function getComparisonList(){
        return array(
            '=' => Yii::t('opportunity', 'equals'),
            '>' => Yii::t('opportunity', 'greater than'),
            '<' => Yii::t('opportunity', 'less than'),
            '<>' => Yii::t('opportunity', 'not equal to'),
            'contains' => Yii::t('opportunity', 'contains'),
            'noContains' => Yii::t('opportunity', 'does not contain'),
            'empty' => Yii::t('opportunity', 'empty'),
            'notEmpty' => Yii::t('opportunity', 'not empty'),
            'list' => Yii::t('opportunity', 'in list'),
            'notList' => Yii::t('opportunity', 'not in list'),
        );
    }

    public function getDefaultRoute(){
        return '/opportunity/opportunity/list';
    }

    public function getDisplayName ($plural=true, $ofModule=true) {
        return Yii::t('opportunity', '{contact} Lists|{contact} List', array(
            (int) $plural,
            '{contact}' => Modules::displayName(false, 'Opportunity'),
        ));
    }
    ...
    public function calculateCount () {
        ...
        return Opportunity::model ()->count ($criteria);
    }
    ...
    public function queryCriteria($useAccessRules = true){
        ...
        if($useAccessRules){
            $accessCriteria = X2Model::model('Opportunity')->getAccessCriteria(); // record-level access control for Opportunity
...
//        );
    ...
    public function campaignDataProvider($pageSize = null){
		$criteria = X2Model::model('Opportunity')->getAccessCriteria();
     ...
     * @param bool $allowNewsletter whether to allow opportunity to be added to a newsletter
     */
    ...
    public function processCriteria(){
        ...
        $contactModel = Opportunity::model();
    ...
    public static function getRoute($id){
        if($id == 'all')
            return array('/opportunity/opportunity/index');
        else if($id == 'new')
            return array('/opportunity/opportunity/newOpportunity');
        else if(empty($id) || $id == 'my')
            return array('/opportunity/opportunity/myOpportunity');
        else
            return array('/opportunity/opportunity/list', 'id' => $id);
    }
    ...
    public function setAttributes($values, $safeOnly = true){
        if($this->type == 'dynamic'){
            ...
            if(count($this->criteriaInput['attribute']) !== count($this->criteriaInput['value'])){
                $this->addError('', Yii::t('opportunity','Invalid list criteria.'));
            }else{
                ...
                if (array_key_exists ('tags', $criteria) && empty($criteria['tags'])) {
                    $this->addError ('tags', Yii::t ('opportunity', 'Tag list must be non-empty'));
                }
            }
        }
