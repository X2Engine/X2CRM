...
Welcome to X2Engine v1.6! X2Engine is a next-generation, open source social 
...
INSTALLATION
-------------------------------------------------------------------------------
1. Eextract and upload X2Engine to the web directory of your choice.
...
LANGUAGES
-------------------------------------------------------------------------------
Most of the included language packs were produced by copy/paste from 
Google Translate and copy/paste. If you have any corrections, suggestions or
custom language packs, please feel free to post them on www.x2community.com
...
- There is an accounts module which is disabled by default and can be turned 
  on by selecting the "Manage Menu Items" link from the admin page.
...
- There is an Email Dropbox which can be set up to run on your server. To do 
  ...
  require_once("/path/to/public_html/x2engine/protected/config/emailConfig.php");

  This dropbox will add any conversations you have with a contact to the 
  comments for that contact if you CC the address. Additionally, if it 
  doesn't recognize the the person you're talking to, it will automatically 
...
- The .htaccess file may cause issues on some servers. If you get a 
  ...
  .htaccess file, the application will work without it.